...
// Billets flottants autour du bocal : plus il y en a, plus on se
// rapproche de l'objectif. Positions fixes, apparition progressive.
const FLOATING_BILLS = [
  { left: '-6%', top: '8%', size: 22, rotate: -18, threshold: 15 },
  { left: '96%', top: '2%', size: 20, rotate: 14, threshold: 30 },
  { left: '-10%', top: '48%', size: 24, rotate: 10, threshold: 45 },
  { left: '100%', top: '42%', size: 22, rotate: -12, threshold: 60 },
  { left: '-4%', top: '82%', size: 20, rotate: -8, threshold: 75 },
  { left: '92%', top: '80%', size: 24, rotate: 16, threshold: 90 },
]
...
// met simplement à jour avec une transition). Des billets apparaissent
// autour au fil de la progression ; à 100%, un clin d'oeil "Wolf of
// Wall Street" façon loup 🐺 (pas de photo d'acteur réel — juste
// l'esprit, pas le visage).
export default function FinanceGoalJar({ objectif, realise }) {
  ...
  const goalReached = pct >= 100 && !isNegative

  return (
    <div className="flex flex-col items-center gap-3">
      <div className="relative">
        {FLOATING_BILLS.map((bill, index) => (
          <span
            key={index}
            className="pointer-events-none absolute select-none transition-all duration-700"
            style={{
              left: bill.left,
              top: bill.top,
              fontSize: bill.size,
              transform: `rotate(${bill.rotate}deg) scale(${pct >= bill.threshold ? 1 : 0})`,
              opacity: pct >= bill.threshold ? 1 : 0,
            }}
          >
            💵
          </span>
      ...
      <div className="text-center">
        ...
        <p className="text-xs text-ink-tertiary">
          {goalReached ? '🐺 Mode Wolf of Wall Street activé' : "de l'objectif atteint"}
        </p>
      </div>
